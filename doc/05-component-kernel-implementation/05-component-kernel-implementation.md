...
The reasons that I chose Sequence:
   I assumed that Rectangle is a data structure that include exactly two elements. The first one
   is width, and the second one is height. We need to have access to the width or the height.
   The adventage of Sequence is that we can get the element at the position we want using entry(int pos).
